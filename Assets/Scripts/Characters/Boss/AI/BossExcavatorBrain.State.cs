...
namespace JunkyardBoss
{
    public sealed partial class BossExcavatorBrain
    {
        private BossExcavatorState SelectAutoState()
        {
            ...
            BossExcavatorState resolvedMoveState = ResolveMoveState(desiredMoveState, targetDistance, baseAngle);

            UpdateStallRuntime(resolvedMoveState, targetDistance);

            if (ShouldBreakStall())
            {
                BossExcavatorAttack aggressiveStallAttack = SelectAggressiveStallAttack(targetDistance, baseAngle, cabinAngle);

                if (aggressiveStallAttack != BossExcavatorAttack.None)
                {
                    _pendingAttack = aggressiveStallAttack;
                    ResetStallRuntime();
